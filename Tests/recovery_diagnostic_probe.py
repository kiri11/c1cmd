...
def main():
    ...
    try:
        ...
        for cycle in range(args.cycles):
            ...
            try:
                ...
                try:
                    ...
                    log('sample-failed', error=repr(error))
                raise
        ...
        # An incomplete restart must not trigger another event or relaunch the app.
        if owns and not restarting:
            try:
                if ae('return count of documents') == 1 and Path(ae('return id of first document')).resolve() == session.resolve():
                    ae('close first document')
                    log('cleanup', openDocuments=ae('return count of documents'), retainedSession=str(session))
            except (RuntimeError, subprocess.TimeoutExpired) as error:
                log('cleanup-failed', error=repr(error), retainedSession=str(session))
