...
## Next recovery campaign

Before repeating qualification, make the recovery harness changes explicit and reviewable:
...
The production runtime and full recovery harness were not changed during these diagnostic runs. The new [diagnostic probe](../../../Tests/recovery_diagnostic_probe.py) makes the bounded controls repeatable. The original failed Session, copied diagnostic Sessions, and complete scratch evidence are retained; no main Catalog or user editing document was used. All ten checked source/copied RAW files retain SHA-256 `2505287a24868a162c94bc879303a76f1e76b7ccb2bba4cacfdfeb4a33c4b868`. Final cleanup confirmed zero open documents.
...
[summary.json](summary.json) identifies every case, fixture, payload hash, database snapshot, and restart result. Case directories retain event streams and read-only database extracts; full binary databases, RAW files, and full process samples remain in scratch storage. Full sample hashes and excerpt paths are in the summary. The packaged CLI hash was `8eac889a502381e0adf95e678807aee9adbeb5c59d9026d871c0f553b0664df1` except the first `nonalias-normal` control, which used the local release executable identified in its environment record.
