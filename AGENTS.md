...
Use `make check` for fast offline feedback and `make qualify` for packaged live workflows. Normal validation excludes deliberate timeout/process-death tests. Deliberate timeout/process-death tests are optional and require an explicit user request. Do not automatically run `make qualify-recovery` or `make qualify-full`, including for recovery-sensitive changes (dispatch, journal, lock, timeout, reconciliation) or release checkpoints. When explicitly requested, `make qualify-recovery` uses an existing archive and `make qualify-full` combines both tiers. Keep live calls sequential. Do not automatically rerun a fault campaign for an ordinary feature or documentation change. Retain mocked failure guards in normal checks and report real recovery coverage separately.
