#!/usr/bin/env python3
"""Exact-build native corrected-lens exploration on a managed clone of a copied RAW.

Usage: lens_probe.py RAW EVIDENCE_DIR. Requires zero open documents. Stops on any
...
for correction in [0, 50, 100]:
    write(f'distortion-{correction}', f'set distortion of lens correction of v to {correction}')
    for angle in [0, 5, -5, 45]:
        write(f'angle-{correction}-{angle}', f'set rotation of adjustments of v to {angle}')
        current=cli('get',ref)
        log('native-bounds', correction=correction, angle=angle, geometry=current['geometry'])
        if angle in [0,5]:
            write(f'native-max-{correction}-{angle}', 'set crop of v to (maximum crop v apply false)')
            preview=cli('preview',ref)
            log('native-preview', correction=correction, angle=angle, preview=preview)
for angle in [0, 5]:
    write('offset-'+str(angle), f'set rotation of adjustments of v to {angle}\nset crop of v to {{2700, 2100, 1500, 2000}}')
    log('offset-bounds', angle=angle, geometry=cli('get',ref)['geometry'])
write('show-distorted-areas', 'set hide distorted areas of lens correction of v to false')
log('exposed-bounds', geometry=cli('get',ref)['geometry'])
