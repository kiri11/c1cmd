...
on run argv
    ...
    tell application "/Applications/Capture One.app"
        ...
        if actionName is "stage" then
            if id of current document is not id of d then error "Current document mismatch"
            ...
            set output sub folder of r to "job-client-death-foreground"
            ...
            marker's writeToFile:"/private/tmp/c1-m0-pending-job-foreground.txt" atomically:true encoding:(current application's NSUTF8StringEncoding) |error|:(missing value)
            delay 30
            return jobId
